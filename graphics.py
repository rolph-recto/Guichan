...
class Graphics:
    ...
    def drawText(self,text,x,y,alignment=0):
        if self.mFont==None:
            raise GCN_EXCEPTION("No font set.")
        
        self.mFont.setAlpha(self.mColor.a)
        if alignment == 0:
            self.mFont.drawText(self,text,x,y)
            
        elif alignment == 1:
            self.mFont.drawText(self,text,x - (self.mFont.getWidth(text)/2),y)
            
        elif alignment == 2:
            self.mFont.drawText(self,text,x - self.mFont.getWidth(text),y)
